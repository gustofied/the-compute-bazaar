"""Windmill script for hourly Vast GPU price ingestion."""
...
def main(
    ...
    env = os.environ.copy()
    _set_env_if_present(env, "VAST_API_KEY", api_key)
    ...
    command = ["/opt/compute-bazaar/.venv/bin/gpu-prices", "ingest-vast"]
    if query is not None:
        command.extend(["--query", json.dumps(query) if isinstance(query, dict) else query])
    if topic_prefix:
