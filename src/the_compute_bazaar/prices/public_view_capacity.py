"""Build the public Akash capacity snapshot."""
...
def akash_capacity_view(
    *,
    ...
    resources = [
        _resource_view(rows, "ALL_GPU", "GPU", "GPU capacity", "GPU units", 1.0),
        _resource_view(rows, "ALL_CPU", "CPU", "CPU capacity", "vCPU", 1_000.0),
    ]
...
def _series_row(row: Mapping[str, Any], scale: float) -> dict[str, Any]:
    return {
        ...
        "total": _scaled(row.get("total_units"), scale),
        "used": _scaled(row.get("rented_units"), scale),
        "available": _scaled(row.get("available_units"), scale),
        "pending": _scaled(row.get("pending_units"), scale),
