...
## Utilization Vocabulary Audit (25 July 2026)

The next research question was whether a reported GPU utilization percentage
means that capacity is leased or that hardware is actively doing work. The
answer depends on the metric, and the bare word was too ambiguous for a public
article.

Primary-source findings:

- Ornn's `volume-metrics` endpoint defines its ratio as the percentage of
  measured GPUs currently rented. It is rental occupancy, not processor
  telemetry. Its public docs do not fully disclose construction of the
  eligible population.
- NVIDIA DCGM separates graphics-engine, SM, tensor, DRAM, occupancy, power,
  and energy measurements. An active warp can still be waiting on memory, and
  high occupancy does not by itself prove effective use.
- Kubernetes schedules CPU and memory requests against node capacity even when
  actual resource usage is low. Allocation and activity can therefore diverge.
- AWS defines EC2 CPU utilization against allocated compute units for one
  instance. It does not report whether that VM is commercially leased.
- E2B exposes five-second CPU, memory, and disk samples for one sandbox. Those
  samples do not reveal provider-fleet occupancy.
- Modal can bill the higher of requested and actual resources, so billed
  quantity is another distinct denominator.
- Inference goodput measures completed requests meeting an explicit service
  objective per unit time. It is closer to useful output than generic GPU
  activity, but only when the output and objective are fixed.
...
Compute Bazaar will not publish a generic `utilization` field. Every future
ratio must name its numerator, denominator, window, scope, source type, and
methodology. Disappearing offers are not inferred to be rentals; running leases
are not inferred to be active; active processors are not inferred to be
productive.

The research was promoted into maintained evidence rather than left in chat:
...
- public payload:
  `utilization`, explicitly marked `methodology_only_no_observed_values`
- allowlisted query:
  `sandbox-cost query --query utilization-ladder`

The article renders the five stages as a compact ledger beside the common-start
comparison. It says plainly that Ornn-style utilization is rented share, while
the present Compute Bazaar data consists of advertised GPU offers, VM rate
cards, and sandbox workload time. No utilization value was invented or
backfilled.

The first v5 build used the authoritative production silver refs from the
14:00 UTC market run and preserved:

- 46 coverage-qualified H100 prints;
- 10 complete seven-vendor VM observations;
- 69 complete sandbox jobs and 690 phase samples;
- five source-linked utilization stages.

After the publication query retained `measurement_family` explicitly, two
independent rebuilds from the same production refs both produced:

- build ID: `sandbox-cost-d009e99a94bb19a9`;
- public payload SHA-256:
  `ea9da92ca2f4959200e4b7b27f3fa2b0ab1e4f0ddf314c188686ebd73ad5f120`;
- 46 H100 prints, 10 complete seven-vendor VM observations, 69 complete jobs,
  690 phases, and five utilization stages.

The checked-in fallback matched those generated bytes. The full 82-test suite,
evidence validation, DataFusion `utilization-ladder` query, Python compilation,
JavaScript syntax check, and diff whitespace checks passed.

Browser QA at 1280 by 720 and 390 by 844 confirmed that the five-stage ledger
stays within the page, collapses to one column on mobile, and retains five
working source links. The document has no page-level horizontal overflow; the
wide audit table remains inside its intended local scroll container. Keyboard
inspection of the seven-vendor VM chart exposed the exact latest median and
range, and its tooltip remained inside the 390-pixel viewport. The browser
