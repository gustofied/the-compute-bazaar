...
| StarSling workload runs | Daily change detection; manual reviewed promotion | Commit-pinned public run captures and source manifest | Run metadata, provider-batch summaries, latest replicate-aligned jobs, and task phases | Latest job distribution, service summaries, phase summaries, and all compatible historical provider-batch summaries | Same-workload runtime and marginal rate-card estimate |
| GPU/sandbox common-start comparison | Rebuilt after each market run | Uses the retained GPU and sandbox evidence above | Uses eligible GPU benchmark history and normalized sandbox prices | Base-100 common-start series and H100 coverage history | Exploratory relative-price chart only |
