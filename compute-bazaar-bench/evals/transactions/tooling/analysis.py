...
def verifier_confirms_missing_output(trial_dir: Path) -> bool:
    try:
    ...
    return (
        details.get("failure_kind") == "invalid_deliverable"
        and "required deliverable not produced" in message
    )
...
def inspect_trajectory(
    ...
    return {
        ...
        "output_visual_render_invocation": any(
            deliverable_lower in command
            and any(marker in command for marker in VISUAL_RENDER_MARKERS)
            for command in lowered_commands
        ),
        "reopened_output": any(
    ...
    *,
    ...
    if not record["artifact_status_ok"]:
        ...
        missing_output_evidence = (
            artifact_failure_has_missing_output_evidence(trial_dir)
            or verifier_confirms_missing_output(trial_dir)
        )
        if (
