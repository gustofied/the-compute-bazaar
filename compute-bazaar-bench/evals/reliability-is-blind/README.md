...
[Harbor environment](harbor/) · [View on Harbor Hub](https://hub.harborframework.com/tasks/gustofied/reliability-is-blind/latest)
...
- One thing to improve: mean reward includes incomplete books at `-1`, while mean delivery rate only includes completed books. The reward makes sense because 95% delivery breaks even and the incomplete penalty stops an agent quitting early, but completion can dominate the score and make the table confusing. A future version should perhaps separate completion and delivery more clearly and say which books each number includes.
